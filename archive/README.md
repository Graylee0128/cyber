...
現行拓樸只有一張：[../demo_network_topology_v0_3.svg](../demo_network_topology_v0_3.svg)。
...
| [demo_network_topology_v0_2_1.svg](./demo_network_topology_v0_2_1.svg) | G2 · 四區隔離 | 目前**已實作**的基線；G3 未落地前，實機跑的是這張 | 缺 Z-EDGE／Z-BLUE 兩區，撐不起 50+ 動態座位與會議中控（見 #65／#76） |
...
G2  v0_2_1                 四區 ← 已實作基線
      ↓ 加入 Z-EDGE 中控平面與 Z-BLUE 防守平面
G3  v0_3                   六區 · 新增契約 5 ← 現行施工依據
      ↓ （尚未施工）
G3+ v3_k8s                 k8s 控制平面 + per-tenant range
```

前四條紫隊契約自 G1 起跨世代不變，是「拓樸可退可換、紫隊不必重做」的依據；第 5 條隨 v0.3 的 Z-EDGE 新增：

1. 三個 port：`:3100` Loki ／ `:9090` Prometheus ／ `:4317` OTLP gRPC
2. `TARGET → MGMT` 單向
3. `RED → MGMT` deny all
4. collector（Alloy／Falco）裝在 target 側，不在 mgmt 側
5. `EDGE → MGMT` deny all（v0.3 新增）
