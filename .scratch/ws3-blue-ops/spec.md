...
**直接後果**：`#37` 的「門檻由 scenario 宣告」要改成「取自平台設定」；
...
那六個位址是 Z-RED 的 kali `.11`–`.16`，**紅隊專屬**；`#34` 明講「不另做帳號登入系統」；
...
給藍隊網段要動 `zones.env`、防火牆、時鐘節點；做帳號系統則是 `#34` 剛拒絕過的東西。
...
**對 `#37` 的影響**：WS5 §1.2 的 Blue Action 契約寫「誰、何時、對哪個 `event_id`」，
...
| 寫哪 | 在哪一區 | 為了什麼 |
|---|---|---|
| Blue Action 記錄 | **Z-APP**（Range Core） | `#37` 的反應時間計分 |
...
| 落在誰 | 什麼事 | 依據 |
|---|---|---|
| **PR #39 / `#17`** | T4 若靠 receiver 自動 enqueue 驅動整條鏈，人在迴圈後要改由測試載具觸發。**PR 正在飛** | §1.1、§1.2 |
