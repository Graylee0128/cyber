...
若不寫死，`#21` 算 hit/miss 時會出現第二套分母，與 WS1 spec §2.1 拒絕的是同一種病。
...
第 2 項不是 WS2 的票，但它**不只影響 `#21`**。第一次落地時只改了後端判定，
2026-08-11 的票體檢掃出另外三張也接不住 —— 記在這裡以免再漏：

| 票 | 漏在哪 | 已補 |
|---|---|---|
| `#21` P2 後端整合 | 第四狀態的判定本身；核心數字公式須排除 `not_executed` | ✅ |
...
| ~~Blue 計分門檻（`Contain < 60 sec`）放哪個欄位~~ | **已由 [WS3 spec §4.3](../ws3-blue-ops/spec.md) 關閉（2026-08-11）：不放進 scenario 檔**，平台級固定。裁決依據正是本檔 §0.1「藍方的東西 scenario 只引用不擁有」—— 該原則反過來否決了 canonical `#36`（原 #37）的既有假設 |
