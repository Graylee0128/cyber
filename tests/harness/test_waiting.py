...
EVENT = {"event_id": "evt-1", "event_type": "attack.detected", "lifecycle": "firing"}
...
class TestFindsTheEvent:
    ...
    def test_ignores_events_that_do_not_match(self):
        other = {"event_id": "evt-other", "event_type": "detection.hit"}
        got = wait_for_event(
...
class TestTimeout:
    ...
    def test_timeout_message_lists_what_was_seen(self):
        clock = FakeClock()
        noise = [{"event_id": f"evt-{i}", "event_type": "detection.hit"} for i in range(3)]
        with pytest.raises(EventNotSeen) as exc:
            wait_for_event(
                _harness(noise), lambda e: False, what="attack.detected", timeout_s=1,
                clock=clock, sleep=lambda s: clock.advance(s),
            )
        message = str(exc.value)
        assert "attack.detected" in message
