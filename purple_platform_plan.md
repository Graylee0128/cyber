...
- [ ] 六台 kali 的來源 IP 在事件中可分辨到個別攻擊者（macvlan 獨立 IP，非 SNAT）
- [ ] Alloy／Falco／response agent 全部部署在 Z-TARGET 側
- [ ] `TARGET → MGMT` 三個 port 通，且反向不通（契約 2 的實測）
- [ ] `RED → MGMT` 實測 deny（契約 3 的實測）
- [ ] 每個來源的四項欄位驗證通過
- [ ] 所有來源完成時間同步驗證
- [ ] SQLi 一條鏈跑通：`attack → log → alert → webhook → agent pull → ipset → 封鎖生效`
- [ ] 事件 schema 定版並交付給三個消費者
- [ ] source registry 可由部署狀態自動產生，P2 可查詢
